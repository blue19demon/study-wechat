...
public class BaiduMapApi {
	...
	public List<BaiduPlace> searchPlace(String query, String lng, String lat) throws Exception {
		// 拼装请求地址
		String api = String.format(baiduMapConfiguration.getUrlPlaceSearch(),query,lat+","+lng, baiduMapConfiguration.getAk());
		// 调用Place API圆形区域检索
	...
	public UserLocation convertCoord(String lng, String lat) {
		// 百度坐标转换接口
		String api = String.format(baiduMapConfiguration.getConvertUrl(),lng, lat);
	...
	public String getLocation(String mjkd_latlng) {
		// 百度位置检索
		String api = String.format(baiduMapConfiguration.getUrlGetLocation(),mjkd_latlng, baiduMapConfiguration.getAk());
		try {
			String jsonCoord = httpRequest(api);
